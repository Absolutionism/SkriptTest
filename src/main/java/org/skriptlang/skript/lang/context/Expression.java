 ...
 * @see SyntaxElement
 */
public interface Expression<Type> extends SyntaxElement, Debuggable, Loopable<Type>, Simplifiable<Expression<? extends Type>> {

	/**
	 * Get the single value of this expression.
	 * <p>
	 * This method may only return null if it always returns null for the given context, i.e. it is equivalent to getting a random element out of {@link #getAll(TriggerContext)} or null iff
	 * that array is empty.
	 * <p>
	 * Do not use this in conditions, use {@link #check(TriggerContext, Predicate, boolean)} instead.
	 *
	 * @param context The context
	 * @return The value or null if this expression doesn't have any value for the context
	 * @throws UnsupportedOperationException (optional) if this was called on a non-single expression
	 */
	@Nullable Type getSingle(TriggerContext context);
	 ...
	 *
	 * @param context the context
	 * @return an {@link Optional} containing the {@link #getSingle(TriggerContext) single value} of this expression for this context.
	 * @see #getSingle(TriggerContext)
	 */
	...
	/**
	 * Get all the values of this expression. The returned array is empty if this expression doesn't have any values for the given context.
	 * <p>
	...
	/**
	 * Gets all possible return values of this expression, i.e. it returns the same as {@link #getArray(TriggerContext)} if {@link #getAnd()} is true, otherwise all possible values for
	 * {@link #getSingle(TriggerContext)}.
	 *
	 * @param context The context
	 * @return An array of all possible values of this expression for the given context which must neither be null nor contain nulls, and which must not be an internal array.
	 */
	...
	/**
	 * Gets a non-null stream for all values of this expression via {@link #getAll(TriggerContext)}.
	 * @param context The context.
	 */
	 ...
	 *
	 * <pre>
	 * return negated ^ {@link #check(TriggerContext, Predicate)};
	 * </pre>
	 *
	 ...
	 */
	boolean check(TriggerContext context, Predicate<? super Type> checker, boolean negated);

	/**
	 * Checks this expression against the given checker. This method must only be used around other checks, use {@link #check(TriggerContext, Predicate, boolean)} for a simple check or the
	 ...
	 * @see SimpleExpression#check(Object[], Predicate, boolean, boolean)
	 */
	boolean check(TriggerContext context, Predicate<? super Type> checker);
	 ...
	 *
	 * @return A supertype of any objects returned by {@link #getSingle(TriggerContext)} and the component type of any arrays returned by {@link #getArray(TriggerContext)}
	 */
	 ...
	 */
	default Class<? extends Type>[] possibleReturnTypes() {
	 ...
	 * <p>
	 * This method significantly influences {@link #check(TriggerContext, Predicate)}, {@link #check(TriggerContext, Predicate, boolean)} and {@link CondIsSet} and thus breaks conditions that use this
	...
	/**
	 * Sets the time of this expression, i.e. whether the returned value represents this expression before or after the context.
	 * <p>
	 ...
	 * @return Whether this expression has distinct time states, e.g. a player never changes but a block can. This should be sensitive for the context (using
	...
	/**
	 * Returns whether this value represents the default value of its type for the context, i.e. it can be replaced with a call to event.getXyz() if one knows the event & value type.
	 * <p>
	...
	@Override
	default Expression<? extends Type> simplify() {
		return this;
	}
	 ...
	 * make sure to return <tt>super.acceptChange(mode)</tt>, and to handle the appropriate ChangeMode(s) in {@link #change(TriggerContext, Object[], ChangeMode)} with
	 * <tt>super.change(...)</tt>.
	 * <p>
	 * Unlike {@link Changer#acceptChange(ChangeMode)} this method may print errors.
	 *
	 * @param mode The mode to check
	 * @return An array of types that {@link #change(TriggerContext, Object[], ChangeMode)} accepts as its <code>delta</code> parameter (which can be arrays to denote that multiple of
	 ...
	 *
	 * @param context The context to use for local variables and evaluation
	 * @param changeFunction A 1-to-1 function that transforms a single input to a single output.
	 *                       Returning null will remove the element.
	 *                       Returning a type not accepted by {@link #acceptChange(ChangeMode)} for {@link ChangeMode#SET}
	 *                       will depend on the implementer. The default implementation will remove the element.
	 * @param <Output> The output type of the change function. Must be a type returned
	...
	default <Output> void changeInPlace(TriggerContext context, Function<Type, Output> changeFunction) {
		changeInPlace(context, changeFunction, false);
	}
	 ...
	 *                       Returning a type not accepted by {@link #acceptChange(ChangeMode)} for {@link ChangeMode#SET}
	 *                       will depend on the implementer. The default implementation will remove the element.
	 * @param getAll Whether to evaluate with {@link #getAll(TriggerContext)} or {@link #getArray(TriggerContext)}.
	 * @param <Output> The output type of the change function. Must be a type returned
	 *              by {{@link #acceptChange(ChangeMode)}} for {@link ChangeMode#SET}.
	 */
	@ApiStatus.Internal
	default <Output> void changeInPlace(TriggerContext context, Function<Type, Output> changeFunction, boolean getAll) {
		...
		for (Type value : values) {
			Output newValue = changeFunction.apply(value);
