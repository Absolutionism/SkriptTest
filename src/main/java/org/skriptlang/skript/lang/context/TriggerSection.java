...
public abstract class TriggerSection extends TriggerItem {
	...
	@Override
	protected final boolean run(TriggerContext context) {
		throw new UnsupportedOperationException();
	}

	@Override
	public abstract @Nullable TriggerItem walk(TriggerContext context);

	protected final @Nullable TriggerItem walk(TriggerContext context, boolean run) {
		debug(context, run);
		if (run && first != null) {
			return first;
		} else {
