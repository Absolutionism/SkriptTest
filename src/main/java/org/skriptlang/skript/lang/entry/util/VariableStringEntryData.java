...
/**
 * A type of {@link KeyValueEntryData} designed to parse its value as a {@link NewVariableString}.
 ...
 */
public class VariableStringEntryData extends KeyValueEntryData<NewVariableString> {

	private final StringMode stringMode;

	/**
	 * Uses {@link StringMode#MESSAGE} as the default string mode.
	 * @see #VariableStringEntryData(String, NewVariableString, boolean, StringMode)
	 */
	public VariableStringEntryData(
		String key, @Nullable NewVariableString defaultValue, boolean optional
	) {
	...
	@Override
	protected @Nullable NewVariableString getValue(String value) {
		// Double up quotations outside of expressions
		if (stringMode != StringMode.VARIABLE_NAME)
			value = NewVariableString.quote(value);
		return NewVariableString.newInstance(value, stringMode);
	}
