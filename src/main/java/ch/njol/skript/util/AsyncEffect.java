...
public abstract class AsyncEffect extends Effect {
	...
	protected TriggerItem walk(Event e) {
		...
		NewVariablesMap localVars = NewVariables.removeLocals(e); // Back up local variables
		// CHANGE
		...
		Bukkit.getScheduler().runTaskAsynchronously(Skript.getInstance(), () -> {
			...
			if (localVars != null)
				NewVariables.setLocalVariables(e, localVars); // CHANGE
			...
			if (getNext() != null) {
				Bukkit.getScheduler().runTask(Skript.getInstance(), () -> { // Walk to next item synchronously
					...
					NewVariables.removeLocals(e); // Clean up local vars, we may be exiting now
					// CHANGE

					SkriptTimings.stop(timing); // Stop timing if it was even started
				});
			} else {
				NewVariables.removeLocals(e); // CHANGE
			}
		});
		return null;
	}
